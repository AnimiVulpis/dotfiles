...
-- beautiful.init(gears.filesystem.get_configuration_dir() .. "themes/vulpis/theme.lua")
...
-- {{{ Key bindings
globalkeys = gears.table.join(awful.key({ modkey }, "s",
    ...
    awful.key({ modkey, "Control" }, "p", -- Menubar refresh
    function()
        menubar.refresh()
        naughty.notify({ text = "reloaded .desktop files" })
    end, { description = "refresh menubar desktop files", group = "launcher" }),
    awful.key({ modkey, "Control" }, "s", -- Lock
    function() awful.util.spawn("loginctl lock-session") end),
    awful.key({ modkey }, ".", -- Layout Machi editor
    function() machi.default_editor.start_interactive() end, {
        description = "edit the current layout if it is a machi layout",
        group = "layout",
    }), awful.key({ modkey }, "/", -- Layout Machi layouter
    ...
    -- Floating clients.
    {
        rule_any = {
            ...
            role = {
                ...
                "WebRTCGlobalIndicator", -- Firefox sharing indicator (WebRTCGlobalIndicator)
            },
        },
